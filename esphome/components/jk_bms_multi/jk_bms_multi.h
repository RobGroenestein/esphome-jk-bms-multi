...
class JkBmsMulti : public Component, public UARTDevice {
 ...
 private:
  ...
  void parse_frame() {
    ...
    // Voltage (voorbeeld: 2 bytes, honderdste volt)
    uint16_t raw_volt = (buffer_[0x10] << 8) | buffer_[0x11];
    float voltage = raw_volt / 100.0f;

    // State of Charge (voorbeeld: 2 bytes, honderdste procent)
    uint16_t raw_soc = (buffer_[0x12] << 8) | buffer_[0x13];
    float soc = raw_soc / 100.0f;

    // Publiceer naar sensoren
    if (voltage_sensors[bms_id] != nullptr) {
      voltage_sensors[bms_id]->publish_state(voltage);
    }
    if (soc_sensors[bms_id] != nullptr) {
      soc_sensors[bms_id]->publish_state(soc);
    }
  }
};
